...
public class OkBuckTask extends DefaultTask {
  ...
  void okbuck() {
    ...
    boolean hasGroovyLib = okBuckExtension.buckProjects.stream().anyMatch(project -> ProjectUtil.getType(project) == ProjectType.GROOVY_LIB);
    if (hasGroovyLib) {
    ...
    boolean hasKotlinLib = okBuckExtension.buckProjects.stream()
        ...
        .anyMatch(type -> type == ProjectType.KOTLIN_LIB || type == ProjectType.KOTLIN_ANDROID_APP || type == ProjectType.KOTLIN_ANDROID_LIB);
